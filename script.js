...
const WITHDRAW_CHANNEL_ID = '-1002991582791'; // Ensure this is correct for your channel
...
async function updateCheckinUI() {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        if (!canClaimToday && currentUser.streak_day >= dayNum && (lastCheckinDate && lastCheckinDate.toDateString() === today.toDateString())) {
            // If already claimed today, mark previous and current day as completed
            capsule.classList.add('completed');
        } else if (canClaimToday && dayNum < streakDay) {
            // If can claim today, mark past streak days as completed
            capsule.classList.add('completed');
        } else if (dayNum === streakDay && canClaimToday) {
            // Current day to claim, make it active
            capsule.classList.add('active');
        } else if (dayNum === currentUser.streak_day && !canClaimToday && (lastCheckinDate && lastCheckinDate.toDateString() === today.toDateString())) {
            // If already claimed today, current day is completed
            capsule.classList.add('completed');
        }
...
document.getElementById('start-ad-task-btn').addEventListener('click', async () => {
    ...
    try {
        ...
        if (typeof show_9725833 !== 'function') {
            throw new Error("Monetag SDK function show_9725833 not found.");
        }
...
⚡ Our team will review this request shortly. Once verified, the status will be updated to ✅ Successful or ❌ Rejected. Stay tuned!`;
        ...
        // In a real app, you would send this to your bot's API endpoint (e.g., via a Firebase Cloud Function):
...
async function renderWithdrawalHistory() {
    ...
    try {
        ...
        withdrawalListDiv.innerHTML = '<p class="info-text" style="color:red;">Failed to load withdrawal history.</p>';
    }
}
...
async function renderTaskHistory() {
    ...
    try {
        ...
        taskListDiv.innerHTML = '<p class="info-text" style="color:red;">Failed to load task history.</p>';
    }
}
...
navItems.forEach(item => {
    item.addEventListener('click', async (event) => { // Made async to allow await calls for UI updates
        ...
        // Ensure currentUser is available before calling specific updates
        if (currentUser) {
            ...
            if (targetScreenId === 'profile-screen') {
                ...
                await renderTaskHistory(); // Default to tasks history
            }
        } else {
