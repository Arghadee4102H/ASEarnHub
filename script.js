...
async function registerOrUpdateUser() {
    ...
    try {
        ...
        document.getElementById('as-points-balance').textContent = '---';
    }
}
...
function updateUI() {
    ...
    if (currentDisplayedBalance !== currentUser.current_balance_as) {
        ...
        document.getElementById('as-points-balance').textContent = (currentUser.current_balance_as || 0).toLocaleString();
    }
    ...
    // Referral Screen
    document.getElementById('my-referral-code').value = currentUser.referral_code || 'Loading...';
    document.getElementById('total-referrals').textContent = (currentUser.referrals_count || 0).toLocaleString();
    ...
    document.getElementById('profile-tg-id').textContent = currentUser.user_id || 'N/A';
    document.getElementById('profile-full-name').textContent = currentUser.telegram_profile_name || 'N/A';
    ...
    document.getElementById('profile-current-points').textContent = (currentUser.current_balance_as || 0).toLocaleString();
    document.getElementById('profile-total-earned').textContent = (currentUser.total_earned_as || 0).toLocaleString();
    document.getElementById('profile-total-tasks').textContent = (currentUser.total_tasks_completed || 0).toLocaleString();
    document.getElementById('profile-streak-day').textContent = (currentUser.streak_day || 0).toLocaleString();
    document.getElementById('profile-total-referrals').textContent = (currentUser.referrals_count || 0).toLocaleString();
...
async function updateCheckinUI() {
    if (!currentUser) {
        ...
        if(streakIndicator) streakIndicator.innerHTML = '<p class="info-text">Streak not available.</p>';
        return;
    }
