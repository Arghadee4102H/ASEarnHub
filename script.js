...
let lastAdCompletionTime = 0; // For hourly rate limiting
...
const WITHDRAW_CHANNEL_ID = '-1002991582791'; // Ensure this is correct for your channel (needs to be a supergroup ID or public channel username for bot to send)
...
if (!tgUser.id) {
    showToast('Telegram user data not available. Please open from the bot.', 'error');
...
async function registerOrUpdateUser() {
    if (!tgUser.id) {
        console.error("No Telegram User ID available for Firebase operations.");
        return;
    }
...
async function fetchUserData() {
    ...
    if (userDoc.exists) {
        ...
        console.warn("User data not found, attempting re-registration.");
...
async function updateUserBalance(points, taskType, reference = null, meta = {}) {
    ...
    const newBalance = currentUser.current_balance_as + points;
    const totalEarned = currentUser.total_earned_as + points;
    const totalTasks = currentUser.total_tasks_completed + 1;
...
function updateUI() {
    ...
    // Home Screen
    document.getElementById('profile-name').textContent = currentUser.telegram_profile_name || currentUser.telegram_username;
    animateBalance('as-points-balance', parseInt(document.getElementById('as-points-balance').textContent), currentUser.current_balance_as);
    ...
    // Referral Screen
    document.getElementById('my-referral-code').value = currentUser.referral_code;
    document.getElementById('total-referrals').textContent = currentUser.referrals_count;
    // For pending referrals, you'd need a more complex query or a dedicated field in user
    // For now, let's keep it simple with just counted referrals.
    // document.getElementById('pending-referrals').textContent = 'N/A';
    if (currentUser.has_submitted_referral) {
        document.getElementById('enter-referral-code').disabled = true;
        document.getElementById('submit-referral-code-btn').disabled = true;
    ...
    // Profile Screen
    document.getElementById('profile-tg-username').textContent = `@${currentUser.telegram_username || 'N/A'}`;
    document.getElementById('profile-tg-id').textContent = currentUser.user_id;
    document.getElementById('profile-full-name').textContent = currentUser.telegram_profile_name;
    document.getElementById('profile-join-date').textContent = currentUser.joined_at ? formatFirestoreTimestamp(currentUser.joined_at) : 'N/A';
    document.getElementById('profile-current-points').textContent = currentUser.current_balance_as.toLocaleString();
    document.getElementById('profile-total-earned').textContent = currentUser.total_earned_as.toLocaleString();
    document.getElementById('profile-total-tasks').textContent = currentUser.total_tasks_completed.toLocaleString();
    document.getElementById('profile-streak-day').textContent = currentUser.streak_day;
    document.getElementById('profile-total-referrals').textContent = currentUser.referrals_count;
    renderTaskHistory(); // Initial render for tasks
}
...
async function updateSummaryUI() {
    ...
    document.getElementById('summary-ads').textContent = `${adsEarnings.toFixed(1)} AS`;
    document.getElementById('summary-tg-tasks').textContent = `${tgTasksEarnings.toFixed(1)} AS`;
    document.getElementById('summary-referrals').textContent = `${referralEarnings.toFixed(1)} AS`;
}
...
async function updateCheckinUI() {
    ...
    let streakDay = currentUser.streak_day;
    ...
    if (lastCheckinDate) {
        ...
        const diffTime = Math.abs(today - lastCheckinDay);
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));

        if (diffDays === 0) { // Already checked in today
            canClaimToday = false;
            checkinMessage.textContent = `You've already claimed your Day ${streakDay} reward today. Come back tomorrow!`;
            ...
            checkinMessage.textContent = `Claim your Day ${streakDay} reward!`;
        } else { // Missed a day
            streakDay = 1; // Reset streak
            nextReward = getCheckinReward(streakDay);
            checkinMessage.textContent = `Streak reset. Claim Day 1 reward to restart!`;
        }
    } else { // Never checked in
        streakDay = 1;
        nextReward = getCheckinReward(streakDay);
        checkinMessage.textContent = `Start your streak! Claim Day 1 reward.`;
    }
    ...
    const checkinRewards = [1, 2, 4, 6, 10, 15, 20];
    ...
    for (let i = 0; i < 7; i++) {
        ...
        if (!canClaimToday && streakDay > dayNum) { // Mark previously completed days
            if (currentUser.streak_day >= dayNum && currentUser.last_checkin_at && currentUser.last_checkin_at.toDate().toDateString() === today.toDateString()) {
                 capsule.classList.add('completed');
            } else if (currentUser.streak_day === 7 && dayNum < 7 && currentUser.last_checkin_at && currentUser.last_checkin_at.toDate().toDateString() === today.toDateString()){
                 // If day 7 completed, all are completed
                 capsule.classList.add('completed');
            } else if (currentUser.last_checkin_at && lastCheckinDate && lastCheckinDate.toDate().toDateString() === today.toDateString() && dayNum <= currentUser.streak_day) {
                 capsule.classList.add('completed');
            }

        } else if (canClaimToday && dayNum < streakDay) {
            capsule.classList.add('completed');
        } else if (dayNum === streakDay && canClaimToday) {
             capsule.classList.add('active'); // Current day to claim
        } else if (dayNum === currentUser.streak_day && !canClaimToday) {
             capsule.classList.add('completed'); // Last claimed day
        }
...
document.getElementById('claim-checkin-btn').addEventListener('click', async () => {
    if (!currentUser || document.getElementById('claim-checkin-btn').disabled) return;

    const today = getTodayUTC();
    const lastCheckinDate = currentUser.last_checkin_at ? currentUser.last_checkin_at.toDate() : null;
    let currentStreakDay = currentUser.streak_day;
...
async function updateAdsTaskUI() {
    ...
    if (todayUTC > lastSeenDayUTC) {
        adsDailyCount = 0;
        await userRef.update({ ads_daily_count: 0 });
        currentUser.ads_daily_count = 0;
    }

    // Reset hourly count if an hour has passed
    if ((now - adsLastHourReset) / (1000 * 60 * 60) >= 1) {
        adsHourlyCount = 0;
        ...
        currentUser.ads_last_hour_reset = firebase.firestore.Timestamp.now();
    }
    ...
    document.getElementById('daily-ads-left').textContent = Math.max(0, MAX_ADS_DAILY - adsDailyCount);
    document.getElementById('hourly-ads-left').textContent = Math.max(0, MAX_ADS_HOURLY - adsHourlyCount);
    document.getElementById('lifetime-ads-completed').textContent = lifetimeAdsCompleted;
...
document.getElementById('start-ad-task-btn').addEventListener('click', async () => {
    if (!currentUser || document.getElementById('start-ad-task-btn').disabled) return;

    // Client-side rate limit check (backend required for true security)
    if (currentUser.ads_daily_count >= MAX_ADS_DAILY || currentUser.ads_hourly_count >= MAX_ADS_HOURLY) {
    ...
    document.getElementById('start-ad-task-btn').disabled = true;
    ...
    try {
        ...
        show_9725833().then(async () => {
            ...
            currentUser.current_balance_as += ADS_REWARD_POINTS;
            currentUser.total_earned_as += ADS_REWARD_POINTS;
            currentUser.total_tasks_completed += 1;
            ...
            updateSummaryUI();
            document.getElementById('start-ad-task-btn').disabled = false; // Re-enable button
        }).catch(error => {
...
async function updateTgTasksUI() {
    ...
    for (const item of taskItems) {
        ...
        if (currentUser.tg_tasks_completed && currentUser.tg_tasks_completed.includes(taskId)) {
            ...
            joinButton.textContent = 'Join ' + (joinButton.querySelector('i') ? '<i class="fas fa-arrow-right"></i>' : '');
        }
    }
}
    ...
    button.addEventListener('click', async (event) => {
        ...
        setTimeout(async () => {
            ...
            if (isVerified) {
                ...
                currentUser.current_balance_as += TG_TASK_REWARD_POINTS;
                currentUser.total_earned_as += TG_TASK_REWARD_POINTS;
                currentUser.total_tasks_completed += 1;
...
document.getElementById('submit-referral-code-btn').addEventListener('click', async () => {
    if (!currentUser || currentUser.has_submitted_referral) {
...
async function creditReferrer(referrerId, referredUserId) {
    ...
    // This requires a more robust tracking mechanism, e.g., a subcollection
    // For simplicity, we'll check if a REFERRAL_CREDIT task exists from referrer to referred.
    ...
    // Notify referrer (if they are online / next time they open the app)
    // For a real-time notification, you'd need Cloud Functions + FCM
    ...
    if (currentUser && currentUser.user_id === referrerId) {
        currentUser.current_balance_as += REFERRER_REWARD_POINTS;
        currentUser.total_earned_as += REFERRER_REWARD_POINTS;
        currentUser.referrals_count += 1;
        updateUI();
    }
}
...
function updateWithdrawUI() {
    ...
    const canWithdrawBinance = currentUser.current_balance_as >= BINANCE_WITHDRAW_POINTS;
    const canWithdrawGooglePlay = currentUser.current_balance_as >= GOOGLE_PLAY_WITHDRAW_POINTS;

    binanceOption.classList.toggle('locked', !canWithdrawBinance);
    googlePlayOption.classList.toggle('locked', !canWithdrawGooglePlay);

    withdrawBinanceBtn.disabled = !canWithdrawBinance;
    withdrawGooglePlayBtn.disabled = !canWithdrawGooglePlay;

    binanceOption.classList.toggle('active-glow', canWithdrawBinance);
    googlePlayOption.classList.toggle('active-glow', canWithdrawGooglePlay);
}

document.getElementById('withdraw-binance-btn').addEventListener('click', async () => {
    if (!currentUser || document.getElementById('withdraw-binance-btn').disabled) return;
...
document.getElementById('withdraw-google-play-btn').addEventListener('click', async () => {
    if (!currentUser || document.getElementById('withdraw-google-play-btn').disabled) return;
    ...
    await requestWithdrawal('GOOGLE_PLAY', GOOGLE_PLAY_POINTS, 0.5, email);
});

async function requestWithdrawal(method, points, usdValue, recipient) {
    if (currentUser.current_balance_as < points) {
    ...
    document.getElementById('withdraw-binance-btn').disabled = true;
    document.getElementById('withdraw-google-play-btn').disabled = true;
    ...
    currentUser.current_balance_as -= points;
    ...
    // Simulate sending to Telegram channel (requires a Telegram bot token and chat ID in a backend)
...
⚡ Our team will review this request shortly. Once verified, the status will be updated to ✅ Successful or ❌ Rejected. Stay tuned!`;
    ...
    // In a real app, you would send this to your bot's API endpoint:
    ...
    // Re-enable buttons
    document.getElementById('withdraw-binance-btn').disabled = false;
    document.getElementById('withdraw-google-play-btn').disabled = false;
    document.getElementById('binance-pay-id').value = '';
    document.getElementById('google-play-email').value = '';
}
...
async function renderWithdrawalHistory() {
    ...
    withdrawalsSnapshot.forEach(doc => {
        ...
        item.className = `withdrawal-item ${withdrawal.status.toLowerCase()}`;
        item.innerHTML = `
...
async function renderTaskHistory() {
    ...
    taskListDiv.innerHTML = '<p class="info-text">Loading task history...</p>';
    ...
    tasksSnapshot.forEach(doc => {
        const task = doc.data();
        const item = document.createElement('div');
        item.className = `withdrawal-item ${task.status.toLowerCase()}`; // Reusing styling
        item.innerHTML = `
...
navItems.forEach(item => {
    item.addEventListener('click', (event) => {
        ...
        // Re-run UI updates for the activated screen if needed
        if (targetScreenId === 'home-screen') updateCheckinUI();
        if (targetScreenId === 'ads-task-screen') updateAdsTaskUI();
        if (targetScreenId === 'tg-tasks-screen') updateTgTasksUI();
        if (targetScreenId === 'withdraw-screen') updateWithdrawUI();
        if (targetScreenId === 'profile-screen') {
            updateUI(); // Full UI update for profile
            renderTaskHistory(); // Ensure tasks are default
        }
    });
});
...
function handleInitialScreen() {
    ...
    if (hash) {
        const foundScreen = document.querySelector(`.nav-item[data-screen="${hash}-screen"]`);
        if (foundScreen) {
            targetScreen = `${hash}-screen`;
        }
    }

    // Activate the corresponding nav item and screen
    navItems.forEach(item => {
        if (item.dataset.screen === targetScreen) {
            item.classList.add('active');
        } else {
    ...
    if (targetScreen === 'home-screen') updateCheckinUI();
    if (targetScreen === 'ads-task-screen') updateAdsTaskUI();
    if (targetScreen === 'tg-tasks-screen') updateTgTasksUI();
    if (targetScreen === 'withdraw-screen') updateWithdrawUI();
    if (targetScreen === 'profile-screen') {
        updateUI(); // Full UI update for profile
        renderTaskHistory(); // Ensure tasks are default
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    await registerOrUpdateUser(); // Ensure user is registered/updated first
    handleInitialScreen(); // Then set the initial screen
});
