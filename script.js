...
const WITHDRAW_CHANNEL_ID = '-1002991582791';
...
// If testing directly in a browser without Telegram, uncomment this block.
// REMEMBER TO COMMENT IT OUT BEFORE DEPLOYING TO GITHUB PAGES FOR PRODUCTION.
/*
if (!tgUser.id) {
    console.warn("Telegram user data not available. Using dummy data for local development.");
    tgUser = {
        id: '123456789', // Use a consistent dummy ID
        first_name: 'Dev',
        last_name: 'User',
        username: 'dev_user',
        language_code: 'en',
...
async function registerOrUpdateUser() {
    if (!tgUser.id) {
        console.error("No Telegram User ID available. Cannot perform Firebase operations.");
        showToast('Please open the app via the Telegram bot for full functionality.', 'error');
        document.getElementById('profile-name').textContent = 'Login Required';
        document.getElementById('as-points-balance').textContent = '0';
        return;
    }
    ...
    try {
        ...
        showToast('Failed to load user data. Please check Firebase rules or connection.', 'error');
    }
}
...
async function fetchUserData() {
    ...
    try {
        ...
        showToast('Failed to retrieve your data. Please check your connection.', 'error');
    }
}
...
function updateUI() {
    if (!currentUser) {
        // console.log('currentUser is null, skipping UI update.');
        document.getElementById('profile-name').textContent = 'Login Required';
        document.getElementById('as-points-balance').textContent = '0';
        return;
    }
    ...
    const currentDisplayedBalance = parseInt(document.getElementById('as-points-balance').textContent.replace(/,/g, '')) || 0;
    if (currentDisplayedBalance !== currentUser.current_balance_as) {
        ...
        document.getElementById('as-points-balance').textContent = currentUser.current_balance_as.toLocaleString();
    }
    ...
    if (currentUser.has_submitted_referral) {
        ...
        document.getElementById('enter-referral-code').disabled = false;
        document.getElementById('submit-referral-code-btn').disabled = false;
...
async function updateCheckinUI() {
    if (!currentUser) {
        ...
        document.getElementById('claim-checkin-btn').textContent = 'Login to Claim';
        document.getElementById('checkin-message').textContent = 'Please log in to start your streak.';
        document.getElementById('streak-indicator').innerHTML = '<p class="info-text">Streak not available.</p>';
        return;
    }
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const lastClaimedDay = currentUser.streak_day;
...
document.getElementById('claim-checkin-btn').addEventListener('click', async () => {
    ...
    try {
        ...
        document.getElementById('claim-checkin-btn').disabled = false; // Re-enable button on error
    }
});
...
async function updateAdsTaskUI() {
    if (!currentUser) {
        ...
        document.getElementById('start-ad-task-btn').textContent = "Login Required";
        document.getElementById('daily-ads-left').textContent = 'N/A';
        document.getElementById('hourly-ads-left').textContent = 'N/A';
        document.getElementById('lifetime-ads-completed').textContent = 'N/A';
        return;
    }
    ...
    try {
        ...
        if ((now.getTime() - adsLastHourReset.getTime()) / (1000 * 60 * 60) >= 1) {
            ...
            // Only update Firestore if a change is needed
            if (currentUser.ads_hourly_count !== 0 || currentUser.ads_last_hour_reset.toDate().getTime() !== now.getTime()) {
        ...
        document.getElementById('start-ad-task-btn').disabled = true; // Disable button on error
    }
}
...
async function requestWithdrawal(method, points, usdValue, recipient) {
    ...
    try {
        ...
        currentUser.current_balance_as -= points; // Update local state immediately
...
⚡ Our team will review this request shortly. Once verified, the status will be updated to ✅ Successful or ❌ Rejected. Stay tuned!`;
        ...
        console.warn("To send this message to Telegram channel " + WITHDRAW_CHANNEL_ID + ", you need a backend server or Firebase Cloud Function to interact with the Telegram Bot API. Client-side JavaScript cannot do this directly for security reasons.");
        ...
        if (currentUser) {
             ...
             currentUser.current_balance_as += points; // Revert local state
             updateUI();
        }
    } finally {
...
async function renderWithdrawalHistory() {
    ...
    try {
        ...
        console.error("Error rendering withdrawal history:", error);
...
async function renderTaskHistory() {
    ...
    try {
        ...
        console.error("Error rendering task history:", error);
...
async function handleInitialScreen() {
    ...
    if (currentUser) { // Ensure currentUser is available after potential registration
        updateUI(); // General UI update
        if (targetScreenId === 'ads-task-screen') await updateAdsTaskUI();
        if (targetScreenId === 'tg-tasks-screen') await updateTgTasksUI();
        if (targetScreenId === 'withdraw-screen') await renderWithdrawalHistory();
